...
Something that has left me slightly perplexed is the blurry line between the domain and the core of a project in most projects I've seen. I don't mean this in an antagonistic way: it's simply a bit foreign to me.
...
First, I recommend using the `src` folder (which Next.js supports without additional configuration), bu not for a specific reason. I prefer to keep code at a different level than configurations and miscellaneous files.

Below, you can find the onion-shaped structure I use for my Next.js projects:

<Image data-shadowless width='1' height='1' src='/assets/images/posts/nextjs-folder-structure-onion.png' /> 
...
It's harder to define a restricted public API when the only separation is a folder. This is why I would recommend setting up `eslint` to prevent such imports, as we will see below.

What are examples of code that belongs to `core`?

Let's assume you use Firebase and need to connect to `Firestore` to fetch data: initializing the database, connecting to it, etc. is what I would regard as being the core of the application. The way the database is used is entirely independent.
...
Why not place them in the same folder, you say? It is to avoid excessive nesting, which can become nasty.

As you may have imagined, here we can place the components that make up our pages. These components are highly tied to the domain and are not supposed to be that reusable across projects. 
...
If you know Next.js, you may not need an explanation for this. `Pages` is a Next-specific folder where we place our routes or pages. 
...
By adding the following configuration to yours, you are automatically warned when you're importing from the wrong paths:
...
This post is a simple take on a complex subject: don't use it for overly complex projects, but also not for hello-world-sized apps. It's for small but still ambitious apps. 
...
For any comments or suggestions, please send me an email. Ciao!