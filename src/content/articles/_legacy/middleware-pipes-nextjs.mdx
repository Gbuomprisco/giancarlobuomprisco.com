...
NB: the concept of _middleware_ in this post is not to be confused with the
recently released [_Next.js Middlewares_](https://nextjs.org/docs/middleware).
Instead, the concept outlined in this article is more akin to _ExpressJS_'s
middlewares.
...
Being used to working with _NestJS_, a batteries-included back-end framework,
...
```tsx
export default authMiddleware(async function myRoute(
  req: NextApiRequest,
  res: NextApiResponse,
) {
...
```tsx
export const withAuthedUser = withMiddleware(withAdmin, withUser);
