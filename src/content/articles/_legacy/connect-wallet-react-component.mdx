...
While being a blockchain engineer involves writing custom _smart contracts_, protocols, etc., it's slightly ironic that web3, as revolutionary as claimed, will still be primarily built using web2 technologies.
...
```tsx
const [address, setAddress] = useState<string>(getSelectedAddress());
```
