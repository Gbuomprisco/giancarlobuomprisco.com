...
I usually follow the Module type approach - but I apply it at the library-level. With the fact the Angular modules _may_ go one day, I feel this is also the most future-proof way to think about it.
...
For example, in the case of a state library, only export state-related entities. That is _mostly actions and selectors_, or a Facade service that hides actions and events from the consumer if you're using one.
...
This suggests your library _may_ be containing more than it should.
...
This is nothing new, and it's well-described in _Clean Architecture_ by Uncle Bob. With that said - it's also really hard to do well in practical terms.
