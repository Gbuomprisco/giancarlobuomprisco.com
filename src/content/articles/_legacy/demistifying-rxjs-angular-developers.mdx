...
const listener = document.addEventEventListener(element, 'input', handler);
```
...
There’s some magic we need to explain there. First of all, we introduced 3 _operators_ and one _creational operator_.
...
```typescript
@Component({
  template: ` <button (click)="onCancel.emit()">Cancel</button> `,
})
