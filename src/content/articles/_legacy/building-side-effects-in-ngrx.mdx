...
export class AssetsEffects {
  ...
  private addAssets() {
    return createEffect(() =>
      this.actions.pipe(
        ofType(getAssetsRequestSuccess.type),
        map(({ payload }: { payload: Asset[] }) => addAssets({ payload })),
      ),
    );
  }
...
export class CoincapService {
  ...
  private createConnection(assets: string[]) {
    ...
    this.webSocket = new WebSocket(EndPoints.WebSocket + `?assets=${assets}`);
  }
}
```
...
export class PricesEffects {
  ...
  private connectPriceStream(assets: string[]) {
    return this.coincap
      ...
      .pipe(takeUntil(this.actions.pipe(ofType(closePriceSubscription.type))));
  }
}
```
