...
```typescript
const value = observable$.pipe(shareReplay({ bufferSize: 1, refCount: true }));
...
That means, if we have an in-flight request (hence, our cache is still missing the value) and another request gets made, instead of executing it, the consumer will receive the cached stream even if it hasn't yet emitted a value. The subject will _multicast_ the resolved value to all its observers.
