...
```typescript
const valueChanges$ = formGroup.valueChanges.pipe(startWith(formGroup.value));
...
```typescript
const allItems$ = this.service.items$.pipe(filter(Boolean));

const doneItems$ = allItems$.pipe(
  map((items) => items.filter((item) => item.done)),
);

const numberOfRemainingItems$ = combineLatest([allItems$, doneItems$]).pipe(
...
```typescript
class UsersDashboardComponent {
  users$ = this.service.users$.pipe(filter(Boolean));
