...
function If<Value = unknown>({ condition, fallback, children }: Props) {
  // body
}
```
...
<CodeSandboxSnippet
  src={`https://codesandbox.io/embed/condescending-tree-r0mki?fontsize=14&hidenavigation=1&theme=dark`}
/>
...
const ReusableComponent: React.FC<{
  ...
  return (
    <div>
      <If condition={heading}>{(Heading) => <Heading />}</If>
    </div>
  );
};
```
