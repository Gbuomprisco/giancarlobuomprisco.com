...
Passing a Subject as an Observer allows to convert the Observable’s behavior from _unicast_ to _multicast_. Using a Subject is, indeed, the only way to make an Observable multicast, which means they will **share the same execution** with multiple Observers.

Wait, though: what does _sharing execution_ actually mean? Let’s see two examples to understand the concept better.
