...
<ng-container [ngSwitch]="state$ | async">
  ...
  <ng-container *ngSwitchCase="state.Success"> All good! </ng-container>

  <ng-container *ngSwitchCase="state.Error"> Ooops! </ng-container>
...
class MyComponent {
  ...
  private readonly initial$ = this.status$.pipe(
    map((status) => status === Status.Initial),
  );

  private readonly pending$ = this.status$.pipe(
    map((status) => status === Status.Pending),
  );
...
type KeysToObservableMapper<State> = {
  readonly [K in Extract<
    keyof State,
    string
  > as `${Lowercase<K>}$`]: Observable<boolean>;
};
...
describe(`reactiveEnum`, () => {
  it('should start with a default value', (done) => {
    ...
    const streams$ = combineLatest([status.loading$, status.initial$]);
