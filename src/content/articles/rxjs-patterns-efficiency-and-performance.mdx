...
numbers$.pipe(filter(predicate)).subscribe(console.log);
// will log 1,2
```
...
textChanges$
  .pipe(filter(Boolean), distinctUntilChanged(), throttleTime(1000))
  .subscribe(console.log);
```
...
textChanges$
  .pipe(filter(Boolean), distinctUntilChanged(), debounceTime(1000))
  .subscribe(console.log);
```
